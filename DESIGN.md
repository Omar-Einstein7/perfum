# Design system — perfume_ahmed_gaber
...
- **Preset:** `cupertino` (Cupertino-style widgets where applicable)
- **Material 3:** Cupertino preset — still uses shared tokens from `lib/src/theme/`.
...
- **Seed color:** `#a3b9f6` (used to derive `ColorScheme` via `ColorScheme.fromSeed` where applicable).
...
Import tokens via `package:perfume_ahmed_gaber/src/theme/theme_constants.dart`.
